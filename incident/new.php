...
<input type="hidden" id="client" value="<?php (isset($row['client_code'])){echo $row['client_code'];}?>">
<input type="hidden" id="report_channel" value="<?php (isset($row['via'])){echo $row['via'];}?>">
<input type="hidden" id="service_type" value="<?php (isset($row['servType'])){echo $row['servType'];}?>">
<input type="hidden" id="category" value="<?php (isset($row['cat'])){echo $row['cat'];}?>">
<input type="hidden" id="product" value="<?php (isset($row['product'])){echo $row['product'];}?>">
<input type="hidden" id="severity" value="<?php (isset($row['flag'])){echo $row['flag'];}?>">
<input type="hidden" id="machine" value="<?php (isset($row['product'])){echo $row['product'];}?>">
...
<div class="col-md-3 form-sidebar">
                    ...
                    <div id="new-incident-alert-container" class="alert alert-info">
                    <i  style="font-size:20px;font-weight:bold" id="new-incident-alert-icon" class="icon-lightbulb pull-left"></i>
                    <span id="new-incident-alert">Please make sure all informations are correct before submitting the incident. </span>
    ...
    $('#incident_description').autosize();
		...
		function validateSN(o, t){
			var z =	$.ajax({
				type	: "POST",
				url		: "validate-snExist.php",
				data	: ({id : o.val()}),
				dataType: "html",
                async	:false
			}).responseText;
			if (z == "false")			{
                ...
                function updateTips(msg){
                        $('#new-incident-alert').text(msg);
                        $('#new-incident-alert-icon').removeClass();
                        $('#new-incident-alert-icon').addClass("icon-exclamation-sign");
                        $('#new-incident-alert-container').removeClass("alert-info");
                        $('#new-incident-alert-container').addClass("alert-danger");
                        $('#new-incident-alert-container').effect("pulsate", { times:2 }, 500);

                    //     $('#new-incident-alert').text("OK!");
                    //     $('#new-incident-alert-icon').removeClass();
                    //     $('#new-incident-alert-icon').addClass("icon-ok");
                    //     $('#new-incident-alert-container').removeClass("alert-info");
                    //   $('#new-incident-alert-container').removeClass("alert-danger");
                    //   $('#new-incident-alert-container').addClass("alert-success");
                    //     $('#new-incident-alert-container').effect("pulsate", { times:2 }, 500);                        
        ...
        function isEmpty( o, msg) {
           ...
           $('#btn_save_incident').click(function(){
                    ...
                    var     project         = $("#incident_project");
                    var     site            = $("#incident_site_id");
                    var     address         = $("#incident_addr_id");
                    var     report_channel  = $("#incident_report_channel");
                    var     contact_person  = $("#incident_contact_person");
                    var     contact_no      = $("#incident_contact_no");
                    var     add_info        = $("#incident_add_info");
                    var     title           = $("#incident_title");
                    var     description     = $("#incident_description");
                    var     sn              = $("#incident_sn");
                    var     service_type    = $("#incident_service_type");
                    var     category        = $("#incident_category");
                    var     machine         = $("#incident_machine");
                    var     sla             = $("#incident_sla");
                    var     severity        = $("#incident_severity");
                    var     recurrence      = $("#incident_recurrence") 

                var bValid = true;
                bValid = bValid && isEmpty( project,"Please choose project");
                bValid = bValid && isEmpty( site,"Please enter relevant customer site");
                bValid = bValid && isEmpty( contact_person,"Please customer name");						
                bValid = bValid && isEmpty( contact_no,"Please customer contact number");						
                bValid = bValid && isEmpty( report_channel,"Please choose report channel");			
                bValid = bValid && isEmpty( title,"Please incident title");						
                bValid = bValid && isEmpty( description,"Please enter detail information regarding the incident");						
                bValid = bValid && isEmpty( service_type,"Please choose relevant service type");
                bValid = bValid && isEmpty( category,"Please choose relevant service category");                					
                if ( bValid ) {
