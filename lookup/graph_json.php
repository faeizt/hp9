...
if ($days < 32) { //daily report
	...
	if(mysqli_query($con,$open)) {
		...
		$x = array();
		$h['label']= $project;
		$result = "SELECT n,days Y,IFNULL(COUNT($date),0) R FROM daily LEFT JOIN v_incident ON DAY($date) = days ".$sql." AND $date BETWEEN (STR_TO_DATE('$begin', '%Y-%m-%d')) AND DATE_ADD((STR_TO_DATE('$end', '%Y-%m-%d')), INTERVAL 1 DAY) GROUP BY days  order by n";
// echo $result;
	...
	if(mysqli_query($con,$open)) {
		...
		$x = array();
		$h['label']=$project;
		$result = "SELECT n,weekno Y,IFNULL(COUNT($date),0) R FROM weekno LEFT JOIN v_incident ON WEEK($date) = weekno  ".$sql." AND $date BETWEEN (STR_TO_DATE('$begin', '%Y-%m-%d')) AND DATE_ADD((STR_TO_DATE('$end', '%Y-%m-%d')), INTERVAL 1 DAY) GROUP BY weekno order by n";
		$q = mysqli_query($con,$result);
		$i=0;
		while($r = mysqli_fetch_array($q)) {
			$i++;
	...
	if(mysqli_query($con,$open)) {
		...
		$x = array();
		$h['label']=$project;
		$result = "SELECT  n,month,MONTHNAME(STR_TO_DATE(month, '%m')) Y,IFNULL(COUNT($date),0) R FROM monthly LEFT JOIN v_incident ON MONTH($date) = MONTH ".$sql."  AND $date BETWEEN (STR_TO_DATE('$begin', '%Y-%m-%d')) AND DATE_ADD((STR_TO_DATE('$end', '%Y-%m-%d')), INTERVAL 1 DAY) GROUP BY MONTH order by n";
		$q = mysqli_query($con,$result);
		$i=0;
		while($r = mysqli_fetch_array($q)) {
			$i++;
	...
	if(mysqli_query($con,$open)) {
		...
		$x = array();
		$h['label']=$project;
		$result = "SELECT n,YEAR Y,IFNULL(COUNT($date),0) R FROM yearly LEFT JOIN v_incident ON YEAR($date) = YEAR  ".$sql." AND $date BETWEEN (STR_TO_DATE('$begin', '%Y-%m-%d')) AND DATE_ADD((STR_TO_DATE('$end', '%Y-%m-%d')), INTERVAL 1 DAY) GROUP BY YEAR order by n";
		$q = mysqli_query($con,$result);
		$i=0;
		while($r = mysqli_fetch_array($q)) {
			$i++;
