...
if(isset($_GET['project_code'])){
	...
	$query = "SELECT id,owner,title,allday, DATE_FORMAT(begin,'%Y-%m-%d') beginallday, DATE_FORMAT(begin,'%Y-%m-%dT%H:%i') begin, DATE_FORMAT(end,'%Y-%m-%dT%H:%i') end FROM events  where owner = '$project_code'";
}
