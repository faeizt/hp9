...
?>
      ...
      <div id="page-wrapper">
              <div class="panel-body">
                <div class="row">
                  <div class="col-lg-12">
                    <h1>Service Type <small>Maintenance Page</small></h1>
                    <ol class="breadcrumb">
                      <li ><a href="../../config"><i class="icon-cogs"></i> Configuration</a></li>
                      <li class="active"><i class="icon-sitemap"></i> Service Type</li>
                    </ol>
                  </div>
                </div><!-- /.row -->
...
$query = "SELECT COUNT(*) as num FROM code_definition where code_cat='servicetyp'";
...
<!--         <td style="width:50px">
          <font face="Arial" size="1">
            <a href="javascript:updateserviceType('<?=$id?>','<?=$code_cat?>','<?=$code?>','<?=$code_name?>','<?=$descr?>')" >
              <span style="cursor: pointer; cursor: hand;" class="ui-icon ui-icon-pencil"></span>
            </a>
            <a href="javascript:deleteserviceType('<?=$id?>')">
